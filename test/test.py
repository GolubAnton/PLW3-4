import unittest
from app import app_counting_words, count_words


class FlaskAppTests(unittest.TestCase):

    def setUp(self):
        self.app_counting_words = app_counting_words.test_client()
        self.app_counting_words.testing = True

    def test_index_route(self):
        response = self.app_counting_words.get('/')
        self.assertEqual(response.status_code, 200)
        self.assertIn('Поиск самого частого слова в файле', response.data.decode())

    def test_result(self):
        with open('../war_and_peace.ru.txt', 'rb') as file:
            response = self.app_counting_words.post('/', data={'file': file})
            self.assertEqual(response.status_code, 200)
